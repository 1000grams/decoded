class SpotifyService {
  ...
  async getRealSpotifyData(artistId = 'ruedevivre') {
    try {
      ...
      const response = await fetch(${this.baseURL}/spotify?artistId=, {
        headers: {
  ...
  async getSpotifyInsights(artistId = 'ruedevivre') {
    try {
      const token = localStorage.getItem('cognito_access_token') || 'demo-token';
      const response = await fetch(${this.baseURL}/spotify/insights?artistId=, {
        headers: {
          'Authorization': Bearer ,
          'Content-Type': 'application/json'
        }
      ...
      if (!response.ok) {
        throw new Error(Spotify Insights API error: );
      }
